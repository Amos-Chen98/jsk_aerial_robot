...
const sensor_msgs::Joy joyParse(const sensor_msgs::Joy& joy_msg)
{
  ...
  if (a_size == PS3_AXIS_SIZE && b_size == PS3_BUTTON_SIZE)
    {
      ROS_INFO_ONCE("[joyParse] PS3 controller detected (axes: %d, buttons: %d)", a_size, b_size);
      joy_cmd = joy_msg;
    }

  if (a_size == PS4_AXIS_SIZE && b_size == PS4_BUTTON_SIZE)
    {
      ROS_INFO_ONCE("[joyParse] PS4 controller detected (axes: %d, buttons: %d)", a_size, b_size);
  ...
  if (a_size == BLT_AXIS_SIZE && b_size == BLT_BUTTON_SIZE)
    {
      ROS_INFO_ONCE("[joyParse] Bluetooth controller detected (axes: %d, buttons: %d)", a_size, b_size);
      joy_cmd.header = joy_msg.header;
      joy_cmd.axes.resize(JOY_AXIS_SIZE, 0);
      joy_cmd.buttons.resize(JOY_BUTTON_SIZE, 0);
      ...
      joy_cmd.axes[JOY_AXIS_BUTTON_REAR_LEFT_2] = joy_msg.axes[BLT_AXIS_BUTTON_REAR_LEFT_2];
      ...
      joy_cmd.axes[JOY_AXIS_BUTTON_REAR_RIGHT_2] = joy_msg.axes[BLT_AXIS_BUTTON_REAR_RIGHT_2];
    }

    if (a_size == ROG1_AXIS_SIZE && b_size == ROG1_BUTTON_SIZE)
    {
      ROS_INFO_ONCE("[joyParse] ROG1 controller detected (axes: %d, buttons: %d)", a_size, b_size);
      joy_cmd.header = joy_msg.header;
      joy_cmd.axes.resize(JOY_AXIS_SIZE, 0);
      joy_cmd.buttons.resize(JOY_BUTTON_SIZE, 0);
  ...
  // Warn if controller type is not recognized
  if (joy_cmd.axes.size() == 0 && joy_cmd.buttons.size() == 0)
  {
    ROS_WARN_ONCE("[joyParse] Unknown controller type (axes: %d, buttons: %d). Supported: PS3(20/17), PS4(14/14), BLT(8/13), ROG1(8/11)", 
                  a_size, b_size);
  }
